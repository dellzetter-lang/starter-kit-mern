...
program
  .name('fsk')
  .description('CLI for MERN Fullstack Starter Kit — scaffold projects and extend them')
  .version('0.1.0');
...
program
  ...
  .option('--preset <variant>', 'Preset: saas|clinic|studio|operations|commerce|custom', 'saas')
  .option('--theme <theme>', 'Design theme: executiveBlue|clinicSoft|studioElevated|operationsDense|commerceWarm')
  .option('--layout <layout>', 'Layout: hybridSaas|sidebarWorkspace|topbarPortal|rightRailStudio')
  .option('--brand-name <name>', 'Brand name')
  .option('--tagline <text>', 'Brand tagline')
  .option('--extra-modules <list>', 'Comma-separated backend modules to include (e.g., users,products)')
  .option('--deploy-targets <list>', 'Comma-separated deploy targets (docker,vercel,railway)')
...
const generateCmd = program.command('generate').description('Add features to existing project');
...
const customizeCmd = program.command('customize').description('Customize project design & branding');
...
// ── Layout ──
const layoutCmd = customizeCmd.command('layout').description('Layout operations');
layoutCmd
  .command('set <layout>')
...
// ── Data display ──
const dataCmd = customizeCmd.command('data').description('Data display template operations');
dataCmd
